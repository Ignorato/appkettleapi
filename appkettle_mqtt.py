...
UDP_IP_BCAST = "10.77.77.255"  # WiFi subnet broadcast
UDP_PORT = 15103
...
class AppKettle:
    ...
    def turn_on(self, temp=None):
        ...
        # Note: wake() should be called by the caller if needed, not here
        # to avoid double-waking and sequence number issues
...
class KettleSocket:
    ...
    def kettle_probe(self):
        ...
        for i in range(1, 4):
            ...
            msg = data.split("#")
            if len(msg) < 7:
                print(f"Unexpected probe response format: {data}")
                rcv_sock.close()
                continue
